...
<nav class="pagination align-right">
    ...
    $firstCurrentPage = $currentPage - 1;
    $endCurrentPage   = $currentPage + 1;

    $endPage = $paginator->total() / $paginator->count();
    ?>

    @if($paginator->onFirstPage())
        <span class="page-numbers current">1</span>
        <a class="page-numbers" href="{{ $paginator->url(2) }}">2</a>
        <a class="page-numbers" href="{{ $paginator->url(3) }}">3</a>
    @else

        @if($currentPage > 2)
            <a class="page-numbers" href="{{ $paginator->url(1) }}">1</a>
            <span  class="page-numbers poins" >...</span>
        @endif

        <a class="page-numbers" href="{{ $paginator->url($firstCurrentPage) }}">{{ $firstCurrentPage }}</a>
        <span class="page-numbers current">{{ $currentPage }}</span>


        @if($paginator->onLastPage() != 1)
            <a class="page-numbers" href="{{ $paginator->url($endCurrentPage) }}">{{ $endCurrentPage }}</a>
        @endif

    @endif
    <?php
        $beforeLast = $paginator->lastPage();
        $beforeLast = $beforeLast - 1;
    ?>
    @if($currentPage != $beforeLast && $currentPage != $paginator->lastPage())
        <span  class="page-numbers poins" >...</span>
